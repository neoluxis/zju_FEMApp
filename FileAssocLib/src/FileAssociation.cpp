...
std::string ReadDefaultValueA(HKEY root, const std::string& subKey) {
  ...
  DWORD type = 0;
  DWORD size = 0;
  if (RegQueryValueExA(hKey, nullptr, nullptr, &type, nullptr, &size) != ERROR_SUCCESS || type != REG_SZ || size == 0) {
    RegCloseKey(hKey);
    return {};
  }

  std::string value(size, '\0');
  if (RegQueryValueExA(hKey, nullptr, nullptr, &type, reinterpret_cast<BYTE*>(&value[0]), &size) != ERROR_SUCCESS || type != REG_SZ) {
    RegCloseKey(hKey);
    return {};
  }
  RegCloseKey(hKey);
...
bool FileAssociation::RegisterAssociation(const std::string& extension,
  ...
  if (RegCreateKeyExA(HKEY_CURRENT_USER, progIdKey.c_str(), 0, nullptr, 0, KEY_WRITE, nullptr, &hKey, nullptr) != ERROR_SUCCESS) {
    return false;
  }
  if (!description.empty()) {
    RegSetValueExA(hKey, nullptr, 0, REG_SZ, reinterpret_cast<const BYTE*>(description.c_str()), description.size() + 1);
  }
  RegCloseKey(hKey);
  ...
  RegSetValueExA(hKey, nullptr, 0, REG_SZ, reinterpret_cast<const BYTE*>(command.c_str()), command.size() + 1);
  RegCloseKey(hKey);
  ...
  RegSetValueExA(hKey, nullptr, 0, REG_SZ, reinterpret_cast<const BYTE*>(progId.c_str()), progId.size() + 1);
  RegCloseKey(hKey);
...
bool FileAssociation::CleanRegister(const std::string& extension,
                                    const std::string& progId,
                                    const std::string& description) {
