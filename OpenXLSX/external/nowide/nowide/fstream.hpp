...
public:
    ...
    internal_buffer_type* rdbuf() const { return buf_.get(); }
    ~basic_ifstream() { buf_->close(); }
...
public:
    ...
    void open(std::string const& file_name,
              std::ios_base::openmode mode = std::ios_base::out | std::ios_base::out) {
        open(file_name.c_str(), mode);
    }
