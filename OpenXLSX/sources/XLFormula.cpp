...
void XLFormulaProxy::setFormulaString(const char* formulaString, bool resetValue)  // NOLINT
{
    ...
    // ===== Remove cell type attribute so that it can be determined by Office Suite when next
...
XLFormula XLFormulaProxy::getFormula() const {
    ...
    if (not formulaNode.attribute("t")
                .empty()) {  // 2024-05-28: de-duplicated check (only relevant for performance,
                             //  xml_attribute::value() returns an empty string for empty
                             //  attributes)
