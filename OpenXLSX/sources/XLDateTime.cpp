...
XLDateTime::XLDateTime(double serial) : m_serial(serial) {
    if (serial < 1.0)
        throw XLDateTimeError(
            "Excel date/time serial number is invalid (must be >= 1.0.)");  // don't permit dates
                                                                            // before
...
XLDateTime::XLDateTime(const std::tm& timepoint) {
    ...
    if (timepoint.tm_mday <= 0 ||
        timepoint.tm_mday > daysInMonth(timepoint.tm_mon + 1, timepoint.tm_year + 1900))
...
std::tm XLDateTime::tm() const {
    ...
    while (serial > 1) {  // 2025-01-10: safeguard against infinite loop (preemptive)
        ...
        if (days + 1 > serial)
            break;  // 2025-01-10: BUGFIX: break on days + 1 > serial (was days >= serial) to
    ...
    while (result.tm_mon < 11) {  // 2025-01-10 BUGFIX: break on December to prevent infinite loop
                                  // after adjusted bugfix below
    ...
    // ===== BEGIN: pass rounded overflow back up the date components. BUGFIX 2025-01-10: added this
    // overflow handling to address issue #138
    if (result.tm_sec >= 60) {
        result.tm_sec -= 60;
        ++result.tm_min;
        if (result.tm_min >= 60) {
