...
constexpr const bool XLRemoveAttributes =
    true;  // helper variables for appendAndSetNodeAttribute, parameter removeAttributes
constexpr const bool XLKeepAttributes = false;  //

/**
 * @brief Get rid of compiler warnings about unused variables (-Wunused-variable) or unused
 * parameters (-Wunusued-parameter)
...
inline XMLNode getRowNode(XMLNode sheetDataNode, uint32_t rowNumber) {
    if (rowNumber < 1 || rowNumber > OpenXLSX::MAX_ROWS) {  // 2024-05-28: added range check
        using namespace std::literals::string_literals;
        throw XLCellAddressError("rowNumber "s + std::to_string(rowNumber) +
    ...
    // ===== If there are now rows in the worksheet, or the requested row is beyond the current max
    // row, append a new row to the end.
    ...
    // ===== If the requested node is closest to the end, start from the end and search backwards.
        ...
        if (result.empty() || (result.attribute("r").as_ullong() != rowNumber)) {
            if (result.empty())
                result = sheetDataNode.prepend_child(
                    "row");  // insert a new row node at datasheet begin. When saving, this will
                             // keep whitespace formatting towards next row node
            else
    ...
    // ===== Otherwise, start from the beginning
        ...
        // ===== At this point, it is guaranteed that there is at least one node_element in the row
        // that is not empty.
        result = sheetDataNode.first_child_of_type(pugi::node_element);

        // ===== It has been verified above that the requested rowNumber is <= the row number of the
        // last node_element, therefore this loop will halt.
...
inline XLStyleIndex getColumnStyle(XMLNode rowNode, uint16_t colNo) {
    ...
    if (not cols.empty()) {
        ...
        while (not col.empty()) {
            if (col.attribute("min").as_int(MAX_COLS + 1) <= colNo &&
                col.attribute("max").as_int(0) >= colNo)  // found
...
/**
 * @brief set the cell reference, and a default cell style attribute if and only if row or column
 * style is != XLDefaultCellFormat
 ...
 * @param colStyles an optional std::vector<XLStyleIndex> that contains all pre-evaluated column
 * styles, and can be used to avoid performance impact from lookup
 */
inline void setDefaultCellAttributes(XMLNode cellNode, const std::string& cellRef, XMLNode rowNode,
                                     uint16_t colNo,
    ...
    if (rowStyle.empty()) {        // if no explicit row style is set
        if (colStyles.size() > 0)  // if colStyles were provided
            cellStyle = (colNo > colStyles.size()
                             ? XLDefaultCellFormat
                             : colStyles[colNo - 1]);       // determine cellStyle from colStyles
    ...
    if (cellStyle != XLDefaultCellFormat)  // if cellStyle was determined as not the default style
                                           // (no point in setting that)
        cellNode.append_attribute("s").set_value(cellStyle);
}

/**
 * @brief Retrieve the xml node representing the cell at the given row and column. If the node
...
inline XMLNode getCellNode(XMLNode rowNode, uint16_t columnNumber, uint32_t rowNumber = 0,
    ...
    // ===== If there are no cells in the current row, or the requested cell is beyond the last cell
    // in the row...
    if (cellNode.empty() ||
        (XLCellReference(cellNode.attribute("r").value()).column() < columnNumber)) {
    ...
    // ===== If the requested node is closest to the end, start from the end and search backwards...
    else if (XLCellReference(cellNode.attribute("r").value()).column() - columnNumber <
             columnNumber) {
        ...
        if (cellNode.empty() ||
            (XLCellReference(cellNode.attribute("r").value()).column() < columnNumber)) {
            if (cellNode.empty())  // If between row begin and higher column number, only
                                   // non-element nodes exist
                cellNode = rowNode.prepend_child(
                    "c");  // insert a new cell node at row begin. When saving, this will keep
                           // whitespace formatting towards next cell node
            else
    ...
    // ===== Otherwise, start from the beginning
        ...
        // ===== It has been verified above that the requested columnNumber is <= the column number
        // of the last node_element, therefore this loop will halt:
 ...
 * @note this function uses a vector of std::string_view because std::string is not
 * constexpr-capable, and in a future c++20 build, a std::span could be used to have the node order
 * in any OpenXLSX class be a constexpr
 */
constexpr const int SORT_INDEX_NOT_FOUND = -1;
inline int findStringInVector(std::string const& nodeName,
...
inline void copyLeadingWhitespaces(XMLNode& parent, XMLNode fromNode, XMLNode toNode) {
    ...
    while (fromNode.type() == pugi::node_pcdata) {  // loop ends on pugi::node_element or node_null
        toNode = parent.insert_child_before(pugi::node_pcdata,
                                            toNode);  // prepend as toNode a new pcdata node
 ...
 * @note 2024-12-19: appendAndGetNode will attempt to perform an ordered insert per nodeOrder if
 * provided Once sufficiently tested, this functionality might be generalized (e.g. in XLXmlParser
 * OpenXLSX_xml_node)
 */
inline XMLNode appendAndGetNode(XMLNode& parent, std::string const& nodeName,
                                std::vector<std::string_view> const& nodeOrder = {},
                                bool force_ns = false) {
    ...
    if (nextNode.empty())
        return parent.prepend_child(
            nodeName.c_str(),
            force_ns);  // nothing to sort, whitespaces "belong" to parent closing tag

    XMLNode node{};  // empty until successfully created;

    int nodeSortIndex =
        (nodeOrder.size() > 1 ? findStringInVector(nodeName, nodeOrder) : SORT_INDEX_NOT_FOUND);
    if (nodeSortIndex != SORT_INDEX_NOT_FOUND) {  // can't sort anything if nodeOrder contains less
                                                  // than 2 entries or does not contain nodeName
        // ===== Find first node to follow nodeName per nodeOrder
        while (not nextNode.empty() &&
               findStringInVector(nextNode.name(), nodeOrder) < nodeSortIndex)
            nextNode = nextNode.next_sibling_of_type(pugi::node_element);
        // ===== Evaluate search result
        if (not nextNode.empty()) {  // found nodeName or a node before which nodeName should be
                                     // inserted
            if (nextNode.name() == nodeName)  // if nodeName was found
                ...
                node = parent.insert_child_before(
                    nodeName.c_str(), nextNode,
                    force_ns);  // insert before nextNode without whitespaces
    ...
    if (node.empty()) {  // neither nodeName, nor a node following per nodeOrder was found
        ...
        nextNode =
            parent.last_child_of_type(pugi::node_element);  // at least one element node must exist,
                                                            // tested at begin of function
        node = parent.insert_child_after(
            nodeName.c_str(), nextNode,
            force_ns);  // append as the last element node, but before final whitespaces
        copyLeadingWhitespaces(parent, nextNode,
                               node);  // duplicate the prefix whitespaces of nextNode to node
    }

    return node;
}

inline XMLAttribute appendAndGetAttribute(XMLNode& node, std::string const& attrName,
...
inline XMLAttribute appendAndSetAttribute(XMLNode& node, std::string const& attrName,
...
/**
 * @brief ensure that node with nodeName exists in parent, has an attribute with attrName and return
 * that attribute
 * @param parent parent node that can perform sibling insertions
 ...
 * @param nodeOrder optional vector of a predefined element node sequence required by MS Office,
 * passed through to appendAndGetNode
 * @returns the requested XMLAttribute or an empty node if the operation failed
 */
inline XMLAttribute appendAndGetNodeAttribute(
    XMLNode& parent, std::string const& nodeName, std::string const& attrName,
    std::string const& attrDefaultVal,
    /**/ std::vector<std::string_view> const& nodeOrder = {}) {
...
/**
 * @brief ensure that node with nodeName exists in parent, has an attribute with attrName, set
 * attribute value and return that attribute
 * @param parent parent node that can perform sibling insertions
 * @param nodename name of the node under which attribute attrName shall exist
 ...
 */
inline XMLAttribute appendAndSetNodeAttribute(XMLNode& parent, std::string const& nodeName,
...
/**
 * @brief special bool attribute getter function for tags that should have a val="true" or
 * val="false" attribute, but when omitted shall default to "true"
 ...
 * @returns true if parent & tagName exist, and attribute with attrName is either omitted or
 * as_bool() returns true. Otherwise return false
 * @note this will create and explicitly set attrName if omitted
 */
inline bool getBoolAttributeWhenOmittedMeansTrue(XMLNode& parent, std::string const& tagName,
