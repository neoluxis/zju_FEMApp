...
private:
    ...
    std::unique_ptr<XMLNode>
        m_cellNode;       /**< The XML node representing the cell currently pointed at. */
...
private:
     ...
     */
    explicit XLRowDataRange(const XMLNode& rowNode, uint16_t firstColumn, uint16_t lastColumn,
...
/**
 * @brief The XLRowDataProxy is used as a proxy object when getting or setting row data. The class
 * facilitates easy conversion to/from containers.
 */
...
public:
    ...
    XLRowDataProxy& operator=(const T& values)  // 2024-04-30: whitespace support
    {
        ...
        if constexpr (std::is_same_v<typename T::value_type, XLCellValue>) {
            ...
            for (auto value = values.rbegin(); value != values.rend(); ++value) {  // NOLINT
                prependCellValue(
                    *value,
                    colNo);  // 2024-04-30: whitespace support: this is safe because only
                             // prependCellValue (with whitespace support) touches the row data
                --colNo;
            }
        }

        // ===== If the container value_type is a POD type, use the overloaded operator= on each
        // cell.
        else {
            auto range = XLRowDataRange(*m_rowNode, 1, values.size(), getSharedStrings());
            auto dst =
                range
                    .begin();  // 2024-04-30: whitespace support: safe because XLRowDataRange::begin
            ...
            while (true) {
                ...
                ++dst;  // 2024-04-30: whitespace support: XLRowDataIterator::operator++ is
                        // whitespace-safe
            }
        }
    ...
    // throw XLInternalError( "templated XLRowDataProxy& operator=(const T& values) instantiated for
    // an XMLNode ("s+ typeid(T).name() +
     ...
     * @details This function calls the convertContainer template function to convert the row data
     * to the container stipulated by the client. The reason that this function is marked explicit
     * is that the implicit conversion operators above will be ambiguous.
    ...
    template <typename Container,
              typename = std::enable_if_t<
                  !std::is_same_v<Container, XLRowDataProxy> &&
                      std::is_base_of_v<std::bidirectional_iterator_tag,
                                        typename std::iterator_traits<
...
private:
     ...
     * @brief Copy constructor.
     * @param other Object to be copied.
     * @note The copy constructor is private in order to prevent use of the auto keyword in client
     * code.
     */
     ...
     * @brief Move constructor.
     * @param other Object to be moved.
     * @note Made private, as move construction should only be allowed when the parent object is
     * moved. Disallowed for client code.
     */
     ...
     * @details This function can convert row data to any user-supplied container that adheres to
     * the design of STL containers and supports bidirectional iterators. This could be std::vector,
     * std::deque, or std::list, but any container with the same interface should work.
     * @tparam Container The container (and value) type to be returned.
     * @return The row data in the required format.
     * @throws bad_variant_access if Container::value type is not XLCellValue and does not match the
     * type contained.
     */
    template <typename Container,
              typename = std::enable_if_t<
    ...
    Container convertContainer() const  // 2024-04-30: whitespace support
    {
        ...
        for (const auto& v : getValues()) {
            // ===== If the value_type of the container is XLCellValue, the value can be assigned
            // directly.
            if constexpr (std::is_same_v<typename Container::value_type, XLCellValue>) *it++ = v;

            // ===== If the value_type is something else, the underlying value has to be extracted
            // from the XLCellValue object.
            // ===== Note that if the type contained in the XLCellValue object does not match the
            // value_type, a bad_variant_access
